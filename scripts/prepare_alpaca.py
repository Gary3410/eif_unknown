...
DATA_FILE_NAME = "llama_vln_parsed_response_v8_easy_train_s2.json"
IGNORE_INDEX = -1
...
def prepare(
    ...
    print("大于512的有:", np.sum(len_list >= 512))
    print("大于1024的有：", np.sum(len_list >= 1024))
    print("最大值", np.max(len_list))
    print("最小值", np.min(len_list))
    """
    for prompt_list_one in prompt_list:
        print(prompt_list_one)
        print("===================")
    exit()
    """
    ...
    torch.save(train_set, file_path.parent / "train_v8_easy_s2.pt")
    if test_split_size > 0:

        print("Processing test split ...")
        test_set = [prepare_sample(sample, tokenizer, max_seq_length, mask_inputs) for sample in tqdm(test_set)]
        torch.save(test_set, file_path.parent / "test_v4.pt")
...
def prepare_sample(example: dict, tokenizer: Tokenizer, max_length: int, mask_inputs: bool = True):
    ...
    # encoded_full_prompt_and_response = tokenize_without_max(tokenizer, full_prompt_and_response, eos=True, max_length=max_length)
...
def generate_prompt(example):
    ...
    if example["input"]:
        return (
            ...
            "Attention, the interactive objects must be in the input.\n\n"
            f"### Instruction:\n{example['instruction']}\n\n### Input:\n{example['input']}\n\n### Response:"
        )
    return (
