...
def main(args):
    ...
    tests_unseen_path = "./data/json_2.1.0/tests_unseen"
    tests_seen_path = "./data/json_2.1.0/tests_seen"
    # valid_unseen_path = "./data/json_2.1.0/valid_unseen"
    # valid_unseen_path = "./data/json_2.1.0/valid_seen"
    instruction_dict_list = []
    split_dict = json.load(open("./data/splits/oct21.json"))
    # tests_unseen_scene_list = split_dict["valid_unseen"]
    # tests_unseen_scene_list = split_dict["valid_seen"]
    tests_unseen_scene_list = split_dict["tests_seen"]
    for tests_unseen_scene_one in tests_unseen_scene_list:
        instruction_dict_one = {}
        task_file_name = tests_unseen_scene_one["task"]
        desc_index = tests_unseen_scene_one["repeat_idx"]
        task_file_path = os.path.join(tests_seen_path, task_file_name, "traj_data.json")
        task_file_dict = json.load(open(task_file_path))
        instruction_list = task_file_dict['turk_annotations']['anns']
        instruction_dict_one["instruction"] = instruction_list[int(desc_index)]["task_desc"]
        instruction_dict_one["scene"] = task_file_dict['scene']["floor_plan"]

        if "task_type" in task_file_dict:
            instruction_dict_one["task_type"] = task_file_dict["task_type"]
        else:
            instruction_dict_one["task_type"] = None
        if "pddl_params" in task_file_dict:
            instruction_dict_one["pddl_params"] = task_file_dict["pddl_params"]
        else:
            instruction_dict_one["pddl_params"] = None

        instruction_dict_one["repeat_idx"] = desc_index
        instruction_dict_one["task_id"] = task_file_dict["task_id"]
        instruction_dict_one["init_action"] = task_file_dict["scene"]["init_action"]
        instruction_dict_list.append(instruction_dict_one)
    ...
    # 初始化保存路径
    base_save_path = "./log_file/llava_s1_s2_vln_parsed_response_procthor_maskrcnn_alfred_test_seen"
    ...
    for val_dict_id, val_dict_one in enumerate(tqdm(instruction_dict_list)):
        # if val_dict_id != 716:
        #     continue
        print("val_dict_id: ", val_dict_id)
        instruction = val_dict_one["instruction"]
        scene = val_dict_one["scene"]
        repeat_idx = val_dict_one["repeat_idx"]
        print(scene)
        print(instruction)
        task_type = val_dict_one["task_type"]
        pddl_params = val_dict_one["pddl_params"]
        task_id = val_dict_one["task_id"]
        print(pddl_params)
        rgb_frame, depth_frame = controller_env.load_scene(house_name=scene)

        if proc_Config.alfred_scene:
            # 需要初始化位置
            init_action = val_dict_one["init_action"]
            initial_position = {"x": init_action["x"], "y": init_action["y"], "z": init_action["z"]}
            rotation = init_action["rotation"]
            horizon = init_action["horizon"]
            event = controller_env.step(action="Teleport", position=initial_position, rotation=dict(x=0, y=rotation, z=0))
            controller_env.perspective_camera_view(reset_view_angle=horizon)
        ...
        for _ in range(4):
            ...
            rgb, depth_frame, mask_list, info_dict = controller_env.to_thor_api_exec(["RotateRight"])
        ...
        # check历史信息
        isin_check = False
        # select_house_index = 0
        # for k, v in scene_result_dict.items():
        #     instruction_one = v["instruction"]
        #     house_name = v["floor_plan"]
        #     if instruction_one == instruction and house_name == scene:
        #         isin_check = True
        #         select_house_index = k
        #         break
        ...
        if not isin_check:
            while not success:
                ...
                try:
                    ...
                    continue
                # 解析具体动作
                random_choice_nav = False
                nav_point, action, target = planner.parse_llava_s2_response(s2_response)
                if np.sum(nav_point) == 0 and try_times > 2:
                    # 随机找一个点
                    target_position = random.choice(controller_env.position_tuple)
                    nav_point = np.asarray(target_position)
                    random_choice_nav = True
                ...
                if target in object_name_list:
                    ...
                    if random_choice_nav:
                        target_nav_position = (nav_point[0], nav_point[-1])
                    else:
                ...
                if len(frontiers_label_dict_list) < 1 and try_times > 5:
                    ...
                    if not is_find:
                        if "openobject" in done_action.done_low_level_action_list[-1].lower() and\
                            ...
                            for nav_point_one in nav_point_list[:3]:
                                ...
                                print("nav to: ", target_nav_position_one)
                    ...
                    if not action_execution:
                        ...
                        if target in ["Microwave"]:
                            if action in ["ToggleObjectOn"]:
                                ...
                                _, _, _, _ = controller_env.to_thor_api_exec(["CloseObject"], object_id=controller_env.interactive_object_id)
                                ...
                                _, _, _, _ = controller_env.to_thor_api_exec(["ToggleObjectOff"], object_id=controller_env.interactive_object_id)
                                result_dict_one["step_action_list"].append(step_action_dict_one)
                ...
                if "goto" in action.lower():
                    ...
                    if target not in controller_env.total_cat2idx.keys():
                        done_action.add_done_action(current_action)
                        done_action.add_low_level_action(action, target)
                        done_list = done_list + current_action + "\n"
                        is_replanning_action = False
        ...
        result_dict_one["house_id"] = scene
