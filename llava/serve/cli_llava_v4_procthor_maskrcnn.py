...
def main(args):
    ...
    # load llava_nav_inter
    _, llava_model_nav_inter, _, _ = load_pretrained_model(args.model_path_nav_inter, args.model_base, model_name, args.load_8bit, args.load_4bit, device=args.device_nav_inter)
    ...
    planner = Planner(
        ...
        llava_model_nav_inter=llava_model_nav_inter
    )
    ...
    for house_index, space_dataset_one in enumerate(instruction_dict_list):
        object_list_one = []
        ...
        house_id2object_list_dict[str(house_index) + "_" + str(house_id_one)] = object_list_one
    ...
    # 初始化保存路径
    base_save_path = "./log_file/llava_s1_s2_nav_inter_vln_parsed_response_procthor_detic_v12_easy_small_attention_with_3D_0708"
    ...
    for val_dict_id, val_dict_one in enumerate(tqdm(instruction_dict_list)):
        # if val_dict_id <= 178:
        #     continue
        # if "house_result_name" in val_dict_one:
        #     continue
        ...
        house_key = str(val_dict_id) + "_" + str(house_id)
        planning_object_name_list = house_id2object_list_dict[house_key]
        ...
        while not success:
            ...
            if instruction_test_time > 30 or controller_env.api_fail_times > 15:
                success = False
                break
            # 统计尝试次数
            ...
            if sem_map.check_is_found_object_in_map(target):
                # 视野中有物体, 则直接选取s1规划
                print("find object, start s1")
                # 直接获取region中最大的候选
                target_nav_position = sem_map.get_sem_map_object_position(target)
                print("old nav position", target_nav_position)
                has_find_object = True
                # goto location的动作应该放到这里
                nav_to_target = True
                if "SliceObject" in done_action.done_low_level_action_list:
                    if "PickUp" in action:
                        if target in done_action.slice_object_name:
                            target_nav_position = done_action.get_sliced_object_position(target)

                if "goto" in action.lower() and target.lower() not in ["fridge", "microwave"]:
                    # 这里重新规划一遍 target_position
                    if target_nav_position is not None:
                        input_dict = {"object_position": target_nav_position,
                                      "object_name": target}
                        response_dict = sem_map.get_nav_inter_feature(input_dict)
                        surround_point_list = response_dict["surround_point_list"]
                        fusion_feature = response_dict["fusion_feature"]
                        new_surround_point_list = []
                        for surround_point_one in surround_point_list:
                            _, path = controller_env.parse_nav_action(surround_point_one)
                            new_surround_point_list.append(np.asarray([path[-1][0], path[-1][-1]]))
                        # 生成nav_inter规划
                        nav_inter_input = planner.trans2_llava_nav_inter_input(target, new_surround_point_list)
                        nav_inter_response = planner.get_llava_response_nav_inter(nav_inter_input, fusion_feature)
                        new_target_nav_position = planner.parse_llava_nav_inter_response(nav_inter_response)
                        if new_target_nav_position is not None:
                            target_nav_position = new_target_nav_position
                        print(nav_inter_response)
            else:
            ...
            if len(frontiers_label_dict_list) < 1 and instruction_test_time > 5:
            ...
            if "goto" in action.lower() and nav_to_target:
                ...
                object_name_list = sem_map.get_object_name_list_from_map()
                if target in object_name_list or stay_here:
                ...
                if action_execution:
                    ...
                    done_action.add_done_action(current_action)
                    done_action.add_low_level_action(action, target)
                    if "SliceObject" in action:
                        robot_position_dict = controller_env.last_event.metadata["agent"]["position"]
                        robot_position_tuple = [robot_position_dict["x"], robot_position_dict["z"]]
                        done_action.slice_agent_position.append(robot_position_tuple)
        ...
        # instruction_response_dict = controller_env.check_complex_task(task_type=task_type, task_label_dict=pddl_params)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device-s1", type=str, default="cuda:4")
    # add llava_nav_inter planner args
    parser.add_argument("--model-path-nav-inter", type=str, default="facebook/opt-350m")
    parser.add_argument("--device-nav-inter", type=str, default="cuda:5")
