...
def main(args):
    ...
    # 初始化保存路径
    base_save_path = "./log_file/llava_s1_s2_vln_parsed_response_procthor_detic_v13_easy_small_attention_0519"
        ...
        while not success:
            ...
            if sem_map.check_is_found_object_in_map(target):
                ...
                if len(object_name_list) > 0:
                    choice_target = random.choice(object_name_list)
                    nav_point_list = sem_map.get_sem_map_object_position_list(choice_target)
                    if len(nav_point_list) > 0:
                        target_nav_position = nav_point_list[0]
                    else:
                        continue
                else:
                    continue
