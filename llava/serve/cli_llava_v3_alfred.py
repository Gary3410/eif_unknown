...
def main(args):
    ...
    for tests_unseen_scene_one in tests_unseen_scene_list:
        ...
        task_file_path = os.path.join(valid_unseen_path, task_file_name, "traj_data.json")
    ...
    # 初始化保存路径
    base_save_path = "./log_file/llava_s1_s2_vln_parsed_response_alfred_tests_unseen"
    if not os.path.exists(base_save_path):
        os.makedirs(base_save_path)

    # -------------------main loop--------------------------
    for val_dict_id, val_dict_one in enumerate(tqdm(instruction_dict_list[21:])):
        ...
        # 首先进行环视
        sem_map.reset()
        controller_env.perspective_camera_view(reset_view_angle=45)
        for _ in range(4):
            # 增加转圈
        ...
        while not success:
            ...
            # 生成第一阶段规划
            new_object_list = controller_env.object_dict2str_llama_s1_only_alfred()
            # 生成第一段input
            ...
            if "goto" in action.lower() or target not in object_name_list:
                ...
                if not is_find:
                    ...
                    for nav_point_one in nav_point_list:
                        target_nav_position_one = sem_map.pixel2world_point(nav_point_one)
                        distance_one = controller_env.goto_location_nav(target_nav_position_one, sem_map)
                ...
                rgb, depth_frame, mask_list, info_dict = controller_env.execAction(action=[
                    action], target_arg=target, target_position=target_nav_position)
                # print(controller_env.last_event)
                if action in ["OpenObject", "CloseObject", "PickupObject", "PutObject"]:
                    hand_object_list = controller_env.last_event.metadata['inventoryObjects']
                    if len(hand_object_list) > 0:
                        info_dict["interactive_object"] = hand_object_list[0]
                _, _ = sem_map.forward(rgb, depth_frame, mask_list, info_dict, task="update")
