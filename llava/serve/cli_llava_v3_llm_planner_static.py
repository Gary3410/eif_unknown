...
def main(args):
    ...
    clip_model.cuda("cuda:3")
    clip_model.eval()

    # load BERT
    bert_tokenizer = BertTokenizer.from_pretrained("./checkpoints/bert-large-uncased")
    lm_model = BertForMaskedLM.from_pretrained("./checkpoints/bert-large-uncased")
    bert_device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
    lm_model.eval()
    lm_model = lm_model.to(bert_device)
    ...
    # 初始化保存路径
    base_save_path = "./log_file/llava_s1_s2_vln_parsed_response_procthor_detic_easy_long_large_llm_planner_0521_static"
    ...
    for val_dict_id, val_dict_one in enumerate(tqdm(instruction_dict_list)):
        ...
        while not success:
            ...
            try:
                ...
                continue
            # 解析具体动作
            _, action, target = planner.parse_llava_s2_response(s2_response)

            # l3mvn planner
            nav_point = sem_map.l3mvn_planner(target)
